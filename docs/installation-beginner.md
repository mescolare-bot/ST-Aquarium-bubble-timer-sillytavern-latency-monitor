...
悬浮图标本身的颜色是你自己选的，两种装法都一样，**不能**用它来判断装成哪一种。
要看的是面板标题右边那个小圆点，鼠标停上去也会写明当前是哪一种：

| 现象 | 说明 |
| --- | --- |
| 圆点是**蓝色**，记录正常出现 | 完整版装好了 |
| 圆点是**绿色**，记录也有 | 跑在精简版上，说明后端没装成功，回第 1 步看 `[3/3]` 有没有报错 |
| 圆点是**红色**，悬浮图标也变红 | 连不上后端，看第 2 步的插件加载日志 |
