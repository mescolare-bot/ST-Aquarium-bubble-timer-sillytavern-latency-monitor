...
> **先读这一段再动手。**
>
> 这篇讲的是**完整版**。完整版**必须修改 SillyTavern 本体的一个源码文件**才能工作，
> 而且必须在跑酒馆的那台机器上执行，因为浏览器写不了 `src/`、`plugins/` 和 `config.yaml`。
>
> **不接受改动酒馆本体，或者不方便碰服务器？** 那就装精简版——在酒馆界面里贴仓库地址即可，
> 只装前端，记录存在你自己的浏览器里。代价是分段耗时会塌缩成一个总数，换设备看不到记录。
> 两种装法的完整对照见 [README](../README.md#安装)。
>
> **完全没碰过命令行？** 先看 [`installation-beginner.md`](installation-beginner.md)，
> 那篇从"怎么打开命令行""怎么登录服务器"开始写，每步都给了预期输出和出错处理。
> 这篇假设你已经能顺利敲命令了。
...
脚本会把文件放好、给本体打上全部补丁、开启服务端插件开关。之后重启酒馆，
再在浏览器里按 `Ctrl + Shift + R` 强制刷新就行。

如果你之前在酒馆界面里贴地址装过精简版，**脚本会自动把那一份删掉**。它和完整版落在两个
不同的目录（精简版的目录名取自仓库名，完整版固定叫 `st-latency-profiler`），两份同时存在的话
酒馆会把扩展加载两遍。浏览器里已经记下的数据不在那个目录里，删目录不会丢，
升级后设置页顶部会提示还剩多少条、可以查看导出或清除。
